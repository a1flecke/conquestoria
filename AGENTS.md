...
Run `eval "$(mise activate bash)"` in a fresh shell before using repo tools. Main commands:

- `yarn dev` starts the Vite dev server.
- `yarn build` runs `tsc` and then produces a production bundle with Vite.
- `yarn test` runs the full Vitest suite once.
- `yarn test:watch` runs Vitest in watch mode during feature work.
