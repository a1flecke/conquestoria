...
Originally twelve phases, twelve PRs. Phases 1 and 2 are prerequisites for everything after; 3–10 are strictly ordered because each removes state that the next would otherwise thread through a deps bag. Phase 12 was added after Phase 6 shipped, once its inline review surfaced a structural issue (#794) that no earlier phase covered — it has no ordering dependency on 7–10 and could in principle run any time after Phase 6, but is listed last since it was discovered last.

**Grown since to Phases 1–14, with Phase 10 split into a seven-part 10b sub-arc.** Phase 10's own review found ~47 functions and the final composition-root wiring had no assigned destination — that became **Phase 10b** (10b-a through 10b-g), all seven now merged (#808, #810, #811, #812, #814, #815, #817). **Phase 13** (`PlayerActionController`, closing test debt Phase 8's own Step 5 optimistically assumed would be free) is merged (#818). Both post-hoc additions are documented in Part VIII below, alongside a 2026-08-14 replan of **Phase 11** and a new **Phase 14**, written after Phase 13 shipped and a full drift check found the original Phase 11 section no longer matched the codebase it was scoped against — see the "Post-hoc replan (Phase 11/14)" note in Part VIII for the specific gaps found and why the phase split the way it did.
...
**`yarn test:ai-playability` already exists and is the better of the two.** `tests/simulation/ai-playability.test.ts` runs 30-turn simulations across real difficulty levels (`'explorer' | 'standard' | 'veteran'`) and AI personality sets via `simulateAIRounds` / `simulateLateEraAIRounds`. It is the existing, maintained answer to "do computer players still work and does difficulty still mean anything." **It is slow** (`run-ai-playability-regressions.sh` allows 300 s with a 120 s per-test timeout), so it runs in the phases that can plausibly affect it — Phase 1 (save/state construction), Phase 9 (turn flow, AI replay, difficulty application), and (**moved from Phase 11 in the 2026-08-14 replan** — the rescoped Phase 11 is pure dead-code/relocation cleanup that touches nothing timing-sensitive) **Phase 14**, whose `commit`-vs-`setStateWithoutRefresh` conversions are exactly the kind of change that can shift when the HUD/AI observe a state update. Phase 12 is UI-blocking-overlay-only and does not touch AI, difficulty, or turn flow, so it does not need this guard.
...
Do **not** exercise judgment about which unrefreshed sites "should" refresh — that is a behavior change, deferred to **Phase 14** (originally slotted as Phase 11 Step 1; split out in the 2026-08-14 replan, see Part VIII). Convert 93 sites in ~6 commits of ~15 each.
...
This is the one source-grep assertion this plan *adds*, and it is legitimate: it measures a debt counter that can only move one direction. **As it turned out, the counter stopped being meaningful well before Phase 11**: it only ever grepped `main.ts`, and extraction phases 5 through 10b-g/13 moved nearly all the real debt into `src/app/controllers/` without anyone widening its scope to follow. Phase 11 deletes the file outright rather than trying to re-target it; the actual debt (66 sites across 12 files, not the ≤46-in-one-file this test tracked) is audited and driven down in **Phase 14** — see Part VIII's 2026-08-14 replan note.
...
### Phase 13 — `PlayerActionController` part 2 ✅ merged (#818)

Found during #787 Phase 8d's review. Phase 8's original Step 5 (pre-split form) listed ten source-grep assertions to convert to real tests; phases 8b–8d converted six as their own targets moved out of `main.ts`, leaving four describe blocks (nine assertions: `player combat wiring`, `shared city founding wiring`, `shared unit upgrade wiring`, `shared city assault wiring`) with an explicit "Out of scope" note in 8d's PR (#799), because their five target functions (`executeAttack`, `foundCityAction`, `executeUpgrade`, `beginPlayerCityAssault`, `executeMinorCivConquest`) were never in either 8c's or 8d's own Files section.

**Deviations from the original plan draft, verified against current code rather than assumed (per `.claude/rules/spec-fidelity.md`):**
- The original draft named a sixth function, `finalizePendingCityCaptureChoice`, as part of this move. It was **not** moved — by the time this phase started it already belonged to `TurnFlowController` (Phase 9) and was never `main.ts`-local, confirmed by `grep` rather than carried forward from the stale draft.
- The original draft assumed `src/app/controllers/player-action-controller.ts` would be created fresh by this phase. **Phase 10b-e shipped first** (for an unrelated function group: `getUnitTurnFlow`, `performWorkerAction`, `performPreach`, `ensurePlayerWarState`, `restAction`, `showEspionageCaptureChoice`) and already created the file — this phase extended it instead, exactly the fallback 10b-e's own docblock had predicted for this ordering.

**Result:** `main.ts` 471 → 168 lines. `PlayerActionController` now owns eleven methods total across both extraction phases. `tests/app/controllers/player-action-controller.test.ts` gained 11 real behavioral tests (combat-seed determinism via same-base-state/varying-turn cloning, post-combat panel-refresh ordering including the city-capture branch, `foundCityInState`/`applyUnitUpgradeToState` delegation, minor-civ conquest's failed-movement short-circuit, city-assault's cannot-occupy and fast-raze-path branches) replacing the four retired describe blocks in `tests/main.integration.test.ts`. Full technique, review, and verification detail is in #818's PR body and the #787 comment thread.

**Known gap carried forward, not introduced by this phase:** the embarked/amphibious-assault branches of `beginPlayerCityAssault`/`executeAttack` still have no dedicated test — the four retired grep blocks never covered them either.
...
**Why `main.ts` doesn't reach `<150` for free:** measured at 168 lines total — 75 are comments (45%), 30 are imports, 8 are blank, leaving 55 lines of real code. Of the 30 import lines, 13 are pre-existing fully-dead imports predating this arc (14 dead symbol names, but `moveUnit`/`getMovementCost` share one line): `moveUnit`, `getMovementCost`, `makePeace`, `visitVillage`, `clearStaleSoloPendingEvents`, `refreshKnownCivilizations`, `getMinorCivNotification`, `registerAllPresentation`, `registerMinorCivNotificationListeners`, `isSpyUnitType`, `isWorkerBusy`, `reconstructLastSeenFromMap`, `createMarketplaceState`, `installGlobalShortcuts` — none of them this arc's problem until now, since this phase is explicitly the final cleanup. Purging all 13 lines brings the file to 168 − 13 = 155 lines — still over. The remaining gap closes by finishing what 10b-f (cross-cutting helpers) and 10b-g (composition root) started: `showNotification` and `maybeShowPendingHoardChoice` are the last two `main.ts`-local functions, and both can move into `bootstrap.ts`'s `createAppComposition` — `showNotification` is a one-line `notifyPlayer(getNotifier(), session, message, type, target)` wrapper `bootstrap.ts` can construct itself using the `getNotifier` accessor it already owns; `maybeShowPendingHoardChoice` only stayed `main.ts`-local because it predates the composition split — it can become a `createAppComposition`-internal sibling function calling `hud.update()` directly instead of `composition.hud.update()`, the same sibling-call pattern Phase 13 already used for `ensurePlayerWarState`/`beginPlayerCityAssault`.

**Files:**
- Create: `tests/app/architecture-boundaries.test.ts`
- Modify: `src/main.ts` (dead-import purge; delete `showNotification`/`maybeShowPendingHoardChoice`; wrap `notifier` in a box so the boundary test's `not.toMatch(/^let /m)` has nothing left to match), `src/app/bootstrap.ts` (absorb the two functions into `createAppComposition`; `AppCompositionDeps` drops `showNotification`/`maybeShowPendingHoardChoice` as external deps), `CLAUDE.md`
- Modify: `src/ui/ui-interaction-state.ts`, `src/app/panel-host.ts`, `tests/ui/keyboard-shortcuts.test.ts`, `tests/ui/desktop-controls.test.ts` (see Step 6b — this needs more than the original draft assumed)
- Delete: `tests/app/refresh-bypass-ratchet.test.ts`

- [ ] **Step 1: The `notifier` `let` needs a box, not just relocation.** `expect(main).not.toMatch(/^let /m)` is a blanket ban with no carve-out in the test as designed, but `notifier` is architecturally load-bearing — the deferred-assignment pattern every phase since 4 has relied on (`GameSessionController.init()` publishes it back via `setNotifier`, well after every controller is constructed). Wrap it: `const notifierBox: { current: Notifier | undefined } = { current: undefined };`, then `getNotifier: () => notifierBox.current!` / `setNotifier: n => { notifierBox.current = n; }` into `createAppComposition`'s existing accessor-pair dep shape — no consumer-facing change, `getNotifier`/`setNotifier` already exist as the crossing-point contract.
- [ ] **Step 2: Move `showNotification` and `maybeShowPendingHoardChoice` into `createAppComposition`.** Verbatim-move technique, same as every prior extraction: `showNotification` becomes a `bootstrap.ts`-internal function built from `getNotifier`/`session` it already has; `maybeShowPendingHoardChoice`'s `composition.hud.update()` becomes a bare `hud.update()` sibling call. Every controller that received either as an injected dep (there are several) keeps working unchanged — only *where* the function is defined moves, not its shape. Run `yarn build` immediately after as the fastest way to catch a missed call site.
- [ ] **Step 3: Purge the 13 pre-existing dead imports** named above from `main.ts`. Re-measure the line count; confirm `<150` before writing the boundary test, not after — if still over, the next-cheapest lever is condensing (not deleting) the longest surviving docblocks, measured (via `awk`, 2026-08-14) at 16 lines (the `composition` construction comment) and 13 lines (the `notifier` binding's own docblock) — the two longest individual blocks in the file. Preserve the load-bearing "why" in each; trim only redundant multi-phase attribution history that's no longer decision-relevant.
- [ ] **Step 4: Write the boundary test** (unchanged from the original draft):
...
- [ ] **Step 5: Write the port-purity test** (unchanged from the original draft — pre-verified against current code during the 2026-08-14 replan: zero violations exist today, every `RenderLoop`/`AudioSystem` import in `src/app/controllers/` is already `import type`, and the only `document.getElementById` text in that directory is inside comments, so this test should pass the moment it's written):
...
- [ ] **Step 6: Delete `tests/app/refresh-bypass-ratchet.test.ts`.** Unlike the original draft, this is not "or lower its bound" — the test only ever grepped `main.ts`, and `main.ts` now has at most one `setStateWithoutRefresh` call regardless of how this phase's other steps land, so any bound is meaningless. The real debt (66 sites across 12 controller files, verified during the 2026-08-14 replan) is Phase 14's job, tracked there instead.
...
- [ ] **Step 6b: Retire `createUiInteractionState`'s factory — this needs more than the original draft assumed.** The original text claimed `context-menu.ts` and two UI test suites "still import" the interface, implying type-only use. Verified during the 2026-08-14 replan: **false** for the two test suites. `tests/ui/keyboard-shortcuts.test.ts` and `tests/ui/desktop-controls.test.ts` both call `createUiInteractionState()` directly (`const interactions = createUiInteractionState();`) to build their own test fixture — deleting the factory breaks both immediately. Sequence:
  1. Inline `createUiInteractionState`'s ~10-line body (a single `blockingOverlayId: string | null` closure) directly into `createPanelHost` in `panel-host.ts`, replacing the `const inner = createUiInteractionState();` composition with local state.
  2. Rewrite both test files' fixture construction to build a plain `UiInteractionState`-shaped object literal inline (`{ setBlockingOverlay: vi.fn(), isInteractionBlocked: () => false }` or equivalent, matched to what each test actually asserts) instead of calling the shared factory.
  3. Delete the factory function from `src/ui/ui-interaction-state.ts`, keeping only the `UiInteractionState` interface export — `context-menu.ts` genuinely only needs the type.
  4. Run those three suites explicitly before committing:

```bash
bash scripts/run-with-mise.sh yarn test tests/ui/keyboard-shortcuts.test.ts tests/ui/desktop-controls.test.ts
```
- [ ] **Step 6c: Run both gameplay guards one final time**, including the slow AI suite:

```bash
bash scripts/run-with-mise.sh yarn test:ai-playability
```
- [ ] **Step 7: Update `CLAUDE.md`.** Add to Architecture:
...
**Files:**
- Modify: whichever of the 12 files above turn out to need a `commit` conversion after Step 1's audit; one test per conversion.

- [ ] **Step 1: Re-run the inventory.** `grep -rn "setStateWithoutRefresh(" src/ --include="*.ts"` fresh — the 66/12-file breakdown above will have drifted further by the time this phase starts, the same way the original "46, mostly in main.ts" estimate drifted. Group by controller, not by call order, so sub-phases (if needed) can be reviewed independently.
- [ ] **Step 2: For each site, determine whether the player could observe stale data.** Convert to `commit` where they could — **each conversion is a bug fix needing its own test and its own line in the PR body**, since these are the only intentional behavior changes this phase is allowed to make (same Global Constraint every other phase in this plan follows). Known candidate from the original survey, still worth checking first since it was never resolved: the pause-menu challenge setters (formerly `main.ts:493, 498`, now wherever that logic lives post-10b-g/13) mutate state with no refresh, which is likely correct (nothing visible changes until the next turn) but must be decided deliberately, not by default.
- [ ] **Step 3: Run the full verification suite** (`yarn build`, `yarn test`, `yarn test:ai-playability` — any conversion that changes refresh timing is exactly the kind of change the AI/difficulty guard exists to catch) after each sub-phase, not just at the end.
- [ ] **Step 4: Close the phase** (or each sub-phase, if split).
...
**Files (audit surface corrected 2026-08-14 — see Part VIII):** the original draft named `main.ts` as a call-site-audit target; verified via `grep -rln "setBlockingOverlay(" src/`, `main.ts` now has **zero**. Eight files match today, two of which are the primitive's own definition sites rather than call sites to audit (`src/ui/ui-interaction-state.ts` — the interface/factory; `src/app/panel-host.ts` — `PanelHost.setBlockingOverlay` wrapping `inner.setBlockingOverlay`). The six genuine call sites are `src/ui/unit-turn-flow.ts`, `src/app/bootstrap.ts` (the `setBlockingOverlay` wrapper function, formerly `main.ts`-local), and four `src/app/controllers/*.ts` files: `player-action-controller.ts`, `turn-flow-controller.ts`, `campaign-entry-controller.ts`, `ceremony-coordinator.ts` — the same extraction-migrated-the-debt pattern Phase 14 documents for `setStateWithoutRefresh`. Re-run the grep before starting Step 1 — this will have drifted further by then.
- Modify: `src/ui/ui-interaction-state.ts` (already partially retired by Phase 11 Step 6b — confirm that landed first, or fold its inlining into this phase's Step 1 if not)
- Modify: `src/app/panel-host.ts`
- Modify (call-site audit, not necessarily every site changes): the six genuine call-site files named above, plus `src/ui/wonder-discovery-queue.ts`, `src/ui/legendary-wonder-completion-queue.ts`
...
**Interfaces:**
- `UiInteractionState`'s remaining consumer, `src/ui/context-menu.ts` (the two test suites no longer construct it after Phase 11 Step 6b rewrites their fixtures to a plain object literal — see that step), must stay drop-in compatible — the same LSP constraint Phase 5 and Phase 11 Step 6b already established for this interface. Whatever the new shape is, `context-menu.ts`'s existing call pattern must keep working unmodified, or `context-menu.ts` gets an explicit, tested migration in this phase.
...
| 13 | `player combat wiring` / `shared city founding wiring` / `shared unit upgrade wiring` / `shared city assault wiring` ✅ retired | 9 |
| — | **Total** | **24** |

**Footnote on "retired":** a small number of these are structural completeness/wiring proofs (e.g. `land-unit water recovery wiring`'s dispatch-site check above, and the `map tap wiring` / `map interaction controller wiring` exhaustive-case-list check that survived Phase 8b through 8d) that stay as `readFileSync`-based grep assertions on purpose even after their behavioral core gets real test coverage — TSC's own exhaustiveness checking and a behavioral test prove different things than "is the live dispatch site still calling the right helper with the right args," which is genuinely awkward to assert any other way for a `main.ts`-local (or moved-but-still-private) function. Treat these as intentionally-retained, not as debt any later phase needs to also chase down.

(24 rather than 21 because three `it` blocks carry multiple `readFileSync` assertions; count from the file, not this table, when checking off. As of Phase 13 shipping, all five rows above are complete — every assertion has either been deleted with real behavioral coverage taking its place, or (the footnote's structural-completeness exceptions) intentionally retained by design. `tests/main.integration.test.ts` still has several describe blocks remaining (`campaign entry wiring`'s e2e-gating check, `land-unit water recovery wiring`, `completed-round AI wiring`'s registrar-wiring proof, `strategic AI city capture wiring`, `map interaction controller wiring`, `selection controller wiring`) — none of them debt against this table; each is either one of the intentionally-retained structural checks or was never one of the 24 to begin with (e.g. the strategic-AI emitter check only ever tested `ai-major-turn.ts`, not `main.ts`).)
...
| 14 | Individually-audited stale-refresh fixes (moved from the original Phase 11 slot — see Part VIII) | one test per conversion |
...
| A phase silently changes refresh timing and the HUD goes stale | **Updated 2026-08-14**: no longer 46 sites in `main.ts` — extraction phases moved the debt into `src/app/controllers/`; verified at 66 sites across 12 files. Which are intentional is not knowable from the code | Phase 2 preserved every site via `setStateWithoutRefresh` at move time; the actual audit-and-convert work is now **Phase 14**, sized and scoped for the real 66/12-file surface instead of the original single-file estimate |
...
| A gameplay/balance change slips in | 5,462 lines of moved code, and `main.ts` calls into nearly every system | Determinism guard (literals, not snapshots) recorded in Phase 1 Step 1 and run in every phase; `yarn test:ai-playability` in Phases 1, 9, and 14 (moved from 11 — see above); `src/systems/` and `src/ai/` are read-only |
...
| Merge conflicts against feature work | `main.ts` is touched by nearly every feature PR | Small PRs merged promptly, not one long branch; no other `main.ts`-touching PR should sit open across a phase merge |
| **Phase 13 touches two already-shipped controllers' construction call sites** | `SelectionController` (8c) and `MapInteractionController` (8d) both took the five moved functions as individual deps; Phase 13 changed both call sites in the same edit | Phase 13 updated both construction sites in one commit, verified by the same full `yarn test` + `yarn build` gate every phase already runs; neither controller's own public interface changed, only what was passed in |
...
| **A plan section goes stale relative to the codebase it describes and nobody notices until it's time to execute it** | Happened to the original Phase 11: written when `setStateWithoutRefresh`/`setBlockingOverlay` debt and the `<150`-line target were all scoped against `main.ts` as a single file; five extraction phases later, both had moved into `src/app/controllers/` and the original text was unexecutable as written | 2026-08-14 replan re-verified every claim in Phase 11/12/14 against the actual current codebase (grep counts, test-file contents, port-purity pre-check) before writing a single line of the new plan text, per `.claude/rules/spec-fidelity.md`; do the same before executing any phase that has sat unexecuted for several other phases' worth of time |
...
- **Spec coverage:** all six controllers from the source list are assigned (`GameSessionController` → Phase 10, `TurnFlowController` → 9, `SelectionController` → 8, `PanelRouter` → 5, `CampaignEntryController` → 10, `PresentationCoordinator` → 7 as twelve registrars). Save-normalization consolidation is Phase 1. `MapInteractionController` (Phase 8), `CeremonyCoordinator` (Phase 6), `HudController` (Phase 10), and `UserSettingsStore` (Phase 4) are added with justification in Part II. `PanelActionsController`/`DiplomacyActionsController` (Phase 10b) and `PlayerActionController` (Phase 10b-e + Phase 13, two separate function groups in one file) round out the roster that Part II's original six-controller inventory didn't anticipate.
- **Completeness:** every one of the 23 module-scope `let`s and every module-scope side effect is assigned a phase in the Part II inventory. Phase 11's `not.toMatch(/^let /m)` is only satisfiable if that table is complete **and** the last surviving `let` (`notifier`) is resolved — Phase 11 Step 1 (2026-08-14 replan) wraps it in a box rather than eliminating the deferred-assignment pattern it represents, since that pattern is genuinely load-bearing, not incidental debt.
- **Placeholder scan:** no TBD/TODO in the plan. The one `// TODO(composition-root)` introduced in Phase 2 Step 5 is a counted, ratcheted debt marker retired in Phase 14 (moved from the original Phase 11 slot — see the 2026-08-14 replan note below).
- **Type consistency:** `GameSession.commit`/`update`/`setStateWithoutRefresh`/`subscribe` are used with those exact names in Phases 2, 8, 9, and 14. `PendingMapIntent` (Phase 3) is consumed by `MapTapIntent`'s `resolve-pending` and `mistap` variants (Phase 8). `SelectionSnapshot` is defined in Phase 3 and consumed by `resolveMapTapIntent` in Phase 8. `PanelId`, `PanelGroup`, `PanelContext`, and `ChoiceAction` are defined in Part II / Phase 5 before first use. `MovementBlockerReason` is imported from its real home, `@/systems/unit-system:986`. `PresentationRegistrar` returns `() => void` in its definition and its disposal tests.
...
- **Deletion audit:** `src/ui/transport-ui-state.ts` has exactly one consumer (`main.ts`) and is safe to delete in Phase 3. `src/ui/ui-interaction-state.ts` has three real consumers as of the 2026-08-14 replan verification — `src/ui/context-menu.ts` (type-only) plus `tests/ui/keyboard-shortcuts.test.ts` and `tests/ui/desktop-controls.test.ts`, which **directly call the factory to build their test fixture**, not just import the interface as originally claimed here. The interface stays; the factory is retired in Phase 11 Step 6b, which now includes rewriting both test files' fixture construction as a required sub-step, not an assumed no-op.
...
- **Post-hoc addition (Phase 12):** this plan originally specified eleven phases. Phase 6's inline review (per #787's no-subagents policy, done inline rather than delegated) traced a hot-seat ceremony leak to its root cause and found a second, structurally deeper issue one level down (`PanelHost`'s blocking-overlay id is a single value, not a stack) that no phase above was scoped to fix. The narrower, confirmed-reachable leak was fixed directly in Phase 6's PR (#793); the structural issue was filed as #794 and added here as Phase 12 rather than expanding #793's blast radius or silently dropping it. All "eleven phases"/"eleven PRs" references above were updated to twelve; Phase 12 has no ordering dependency on Phases 7–10 and is appended at the end only because it was discovered last. Its own Files section was corrected in the 2026-08-14 replan (see below) — the `main.ts` call-site audit it originally named no longer exists as such.
- **Post-hoc addition (Phase 13):** Phase 8's own Step 5 (in its pre-split form) optimistically listed ten source-grep assertions to convert to real tests, without checking whether their target functions were actually in either sub-phase's Files section. By the time Phase 8d's inline review reached that step, five of the ten functions involved (`executeAttack`, `foundCityAction`, `executeUpgrade`, `beginPlayerCityAssault`, `executeMinorCivConquest` — a sixth, `finalizePendingCityCaptureChoice`, was originally assumed to belong here too but turned out to already live in `TurnFlowController` since Phase 9, caught during Phase 13's own execution) had never moved — they are `main.ts`-local dependencies `SelectionController` (8c) and `MapInteractionController` (8d) call, not files either phase creates or moves. Rather than silently expand 8d's PR ([#799](https://github.com/a1flecke/conquestoria/pull/799)) to also extract them, the nine still-unconverted assertions were left in place with an explicit "Out of scope" note in that PR, and added here as Phase 13 — a dedicated phase to extract the remaining `PlayerActionController` functions and finish the conversion Phase 8's Step 5 assumed would be free. Phase 13 depends on 8c and 8d (both already shipped) and has no ordering dependency on 9–12. **Shipped as #818** — see Phase 13's own section above for the two verified deviations from the original draft.
- **Post-hoc replan (Phase 11/14), 2026-08-14:** after Phase 13 merged, `main.ts` landed at 168 lines — close to but over the original `<150` target — prompting a full drift check of the original Phase 11 section against the actual current codebase before executing it (per `.claude/rules/spec-fidelity.md`: verify claims about current code state directly rather than carrying a written plan forward on trust). Four real gaps were found, all now fixed in the rewritten sections above: (1) the `<150` line target had no achievable path in the original text — a full dead-import purge only reaches ~155 lines; closing the rest requires relocating `showNotification`/`maybeShowPendingHoardChoice` into `bootstrap.ts`, not mentioned anywhere in the original plan; (2) `refresh-bypass-ratchet.test.ts` only ever grepped `main.ts`, so it silently stopped tracking real debt once extraction phases moved `setStateWithoutRefresh` calls into controllers — the true count is 66 sites across 12 files (was assumed ~46-48 in one file), now **Phase 14**, decoupled from Phase 11 since the boundary test itself never depended on it; (3) the boundary test's `not.toMatch(/^let /m)` has no carve-out for the architecturally load-bearing `notifier` `let`, needing an unplanned box-wrapper fix; (4) Phase 11 Step 4b (now 6b) understated its own blast radius — the two UI test suites it named call the factory directly, not just the type, so deleting it breaks both without an unplanned fixture rewrite. Phase 12's Files section had the same class of drift (a `main.ts` `setBlockingOverlay` audit surface that no longer exists) and was corrected in the same pass. The stale, already-merged-content-reverting docs-only PR #800 (which had drafted Phase 13's section from a branch cut before Phase 10b existed) was closed as superseded rather than merged.
