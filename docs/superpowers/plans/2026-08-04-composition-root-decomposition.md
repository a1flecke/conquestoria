...
- Modify: `src/ui/ui-interaction-state.ts`, `src/app/panel-host.ts`, `tests/ui/keyboard-shortcuts.test.ts`, `tests/ui/desktop-controls.test.ts` (see Step 4b — this needs more than the original draft assumed)
...
- [ ] **Step 3: Purge the 13 pre-existing dead imports** named above from `main.ts`. Re-measure the line count; confirm `<150` before writing the boundary test, not after — if still over, the next-cheapest lever is condensing (not deleting) the longest surviving docblocks (the `composition` construction comment and the primitives block are the two largest), preserving the load-bearing "why" in each, trimming only redundant multi-phase attribution history that's no longer decision-relevant.
...
**Files (audit surface corrected 2026-08-14 — see Part VIII):** the original draft named `main.ts` as a call-site-audit target; verified via `grep -rn "setBlockingOverlay(" src/`, `main.ts` now has **zero** — all 26 real call sites live in `src/ui/unit-turn-flow.ts` and six `src/app/controllers/*.ts` files (`player-action-controller.ts`, `turn-flow-controller.ts`, `campaign-entry-controller.ts`, `ceremony-coordinator.ts`, plus `panel-host.ts`/`bootstrap.ts` themselves), the same extraction-migrated-the-debt pattern Phase 14 documents for `setStateWithoutRefresh`. Re-run the grep before starting Step 1 — this will have drifted further by then.
...
- Modify (call-site audit, not necessarily every site changes): `src/ui/unit-turn-flow.ts`, the six controller files named above, `src/ui/wonder-discovery-queue.ts`, `src/ui/legendary-wonder-completion-queue.ts`
