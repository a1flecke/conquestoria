...
> **Status: ✅ executed.** All 11 tasks completed inline; full test suite (9158 tests) and `yarn build` pass clean. Task 11 Step 3 (manual browser sanity pass) was explicitly skipped per its own stated allowance — reaching the "Arms Control Treaty NP built" state via console/save-edit was not practical in this session, and the jsdom-based tests in Tasks 8 and 10 already assert the exact rendered DOM text under precise state conditions. Two gaps not caught by the design/plan review passes were found and fixed during execution: `DiplomaticAction` (core/types.ts) was missing `'arms_control_pact'`, and `getActiveArmsControlCap` needed a defensive `?? []` fallback for `civ.diplomacy.treaties` once it was threaded into widely-shared call sites (`ai-production.ts`, `planning-system.ts`) that many pre-existing test fixtures build without a full `treaties` array. A post-execution inline review (same dimensions as the design/plan reviews, run against the real diff) then caught a significant bug: `getAvailableActions`'s `arms_control_pact` gate lacked the not-already-signed-with-this-civ check every other treaty type has, which would have grown an unbounded duplicate treaty entry every AI turn once conditions were met (AI<->AI signs immediately, unlike the human-target propose-and-wait branch). Fixed with the same guard pattern as the other treaty types, plus a direct `getAvailableActions` regression and a two-consecutive-`processAITurn` regression.
