...
- Extend the explicit fighting-line chain to
  ...
  when the complete three-technology Missile Cruiser gate is satisfied.
...
Missile Cruiser declares the existing typed `airDefenseProvider` capability with radius
1, a flat `+10` defense modifier, and the existing air-defense stacking group. The
canonical coverage resolver continues to select the strongest available provider, rather
than summing overlapping Missile Cruisers or combining Missile Cruiser with weaker
Mobile Anti-Air or Anti-Air Battery coverage.
...
the exact +10 radius-one air-defense value, the three technology requirements, the
explicit upgrade direction, and submarine vulnerability through the existing expandable
unit, production, and combat-fact surfaces.
...
catalogues as human players. Its research, coastal-city production, and fleet-escort
positioning use owned state and earned observations only. The escort helper continues
to target visible aircraft threats; hidden aircraft cannot create an action or alter
the ranking. Explorer, Standard, and Veteran retain identical unit values, legality,
combat math, coverage, and information boundaries; difficulty may only influence
already-typed decision-quality fields.
...
prerequisite negative; range-three fire support; capital-ship submarine counter and
non-capital negative; radius-one +10 coverage; strongest-source facts; non-air and
out-of-range negatives; AI research/production/visible-threat escort behavior; human
and non-human shared combat paths; solo and two-human viewer isolation; and
save/queue round trips with no schema change.
