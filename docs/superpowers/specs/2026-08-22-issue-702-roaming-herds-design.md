...
adjacent tiles and chooses an outward two-step route with a stable seeded tie-break. It
returns a plain route fact; the herd-turn caller commits that fact to state. The renderer
consumes only the committed fact through a viewer-scoped presentation helper, never by
re-running pathfinding.

The existing `CrisisForce` registry remains its ownership and target boundary. This issue
extends it only with optional, serializable herd-route records keyed by herd unit ID. A
...
or another force's records.

## Routing contract

1. A herd route consists of at most two adjacent map coordinates. The herd moves at most
   two hexes when a later crisis turn consumes the route; this issue establishes and
   persists the route calculation without creating the #703 active-crisis scheduler.
2. A candidate tile must exist, must not be water or a mountain, and must not be a city
   center. It must be unoccupied by a map unit for ordinary routing. No route may cross an
   illegal intermediate tile to reach a legal second tile.
3. “Outward” is measured from the target civilization's nearest city center. Each step
   prefers increasing hex distance from that center; a legal non-increasing step is used
   only when no increasing legal candidate exists. This creates a legible escape pressure
   rather than pursuit of a player unit or city.
4. Every legal candidate has a deterministic total cost: outward progress first, then a
   bounded avoidance score. Avoidance derives only from visible defensive facts: a
   completed Fort or Citadel tile and an adjacent fortified land military unit. Multiple
   defensive contributors to one candidate saturate at a named per-hex maximum, so screens
   help but cannot produce an impassable wall.
...
6. Equal-cost candidates use a stable seeded tie-break derived from `gameId`, force ID,
   herd unit ID, and turn. Iteration order, object insertion order, and renderer calls
   cannot change the selected route.
...
When a herd is visible to its target civilization, the map can show the first two
committed route hexes with an icon plus the plain text label “Herd path: next two steps.”
The overlay is informational, non-interactive, honors reduced-motion preferences, and
does not use color, animation, or sound as its only signal.

The viewer-scoped helper returns no route for a non-target civilization, for a target that
cannot currently see the herd, or for a stale/malformed record. At a hot-seat handoff the
renderer receives the new `currentPlayer` and immediately drops the former player's path;
it does not recalculate a route. This preserves the previous preview until the herd's next
...
optimizers and future AI response code. Future #704 AI consumes the same committed route
and visible screen facts rather than predicting hidden routes.

## Save compatibility and errors

The implementation will re-audit the current schema number immediately before editing. If
the persisted `CrisisForce` shape changes, it will add the next numbered migration and
normalize schema-0, prior-version, current-version, malformed, and mid-route saves
...
- Fort/Citadel tests proving a screen raises bounded avoidance, a selected Fort/Citadel
  ends movement after one step, and multiple contributors cannot exceed the per-hex cap.
...
- Presentation tests for a visible target, unseen herd, a different hot-seat viewer, and
  handoff. Rendered output must contain the path label when allowed and contain neither
  coordinates nor markers when not allowed.
