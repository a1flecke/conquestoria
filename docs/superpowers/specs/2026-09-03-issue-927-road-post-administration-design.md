...
Reuse a parameterized form of the existing capital-road BFS, rather than create another route model. For administration only, a traversed road tile must have `hasRoad === true` and `tile.owner === city.owner`; own city centers are traversable endpoints. The path starts at the owner’s capital and must reach the target city center.
...
- A gap, pillage, ownership loss, ocean/coast gap, or disconnected island breaks the route immediately.
...
| Distance row | Road-only relief | Courthouse relief | Both rows | Remaining sprawl |
|---:|---:|---:|---:|---:|
| 2 | 0 | 1 | -1 | 1 |
| 8 | 3 | 4 | -6 | 2 |
| 14 | 5 | 7 | -12 | 2 |
...
1. A meaningfully distant city without the tech gets a research-first Military Logistics recommendation when that research is plausible.
...
`UnrestReliefSource` gains minimal generic metadata for a tech-gated, infrastructure-derived source. AI research scores the declared tech only when the AI has sufficiently pressured, distant, disconnected own cities. There is no `military-logistics` ID branch, `currentPlayer` use, AI-only relief, or difficulty exception. Existing road-building gives the AI the same legal path to benefit.
...
| Computer players | Existing deterministic road planning is shared and owner-scoped. Research valuation uses generic source metadata, not a cheat or one-off strategy. |
...
| Performance | BFS is O(map tiles), deterministic, and bounded. Multi-city pressure evaluation must share one derived connectivity result per civ/evaluation pass instead of city×BFS scans. |
...
- Cover pre-unlock research guidance, post-unlock connection guidance, connected non-repetition, and Courthouse fallback.
...
- Add bounded traversal/call-sharing coverage for multi-city pressure evaluation.
