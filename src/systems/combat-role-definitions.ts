...
export const UNIT_ROLE_DEFINITIONS = {
  ...
  horseman: role('shock', 'Fast mounted attacker that flanks exposed ranged units.', ['mobile', 'capture'], { counters: ['ranged', 'siege'], vulnerableTo: ['anti-mounted'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-light-support' }),
  chariot: role('shock', 'Heavy early charger gains 20% on open ground but loses 15% in rough terrain.', ['mobile', 'capture'], { counters: ['ranged', 'siege'], vulnerableTo: ['anti-mounted'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-heavy' }),
  cavalry: role('shock', 'Fast pursuit cavalry strikes weakened enemies before they recover.', ['mobile', 'capture'], { secondaryRoles: ['pursuit'], counters: ['ranged', 'siege'], vulnerableTo: ['anti-mounted'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-light-support' }),
  armored_car: role('reconnaissance', 'Fast reconnaissance car pursues damaged foes but cannot hold enemies in place.', ['recon', 'mobile'], { secondaryRoles: ['pursuit'], counters: ['ranged', 'siege'], vulnerableTo: ['frontline'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-light-support', domainTransitionReason: 'Succeeds into air assault and reconnaissance at a Helicopter Base.', publicFacts: ['+15% attack against targets below 60 HP', 'No zone of control'] }),
  knight: role('shock', 'Armored mounted attacker that overwhelms vulnerable land units.', ['mobile', 'capture'], { counters: ['ranged', 'siege'], vulnerableTo: ['anti-mounted'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-heavy' }),
  cuirassier: role('shock', 'Armored cavalry breaks open ground but is slower than Cavalry.', ['mobile', 'capture'], { counters: ['ranged', 'siege'], vulnerableTo: ['anti-mounted'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-heavy', publicFacts: ['+15% attack on open ground', 'Requires Horses and Iron'] }),
  ...
  catapult: role('siege', 'Slow bombard unit that damages cities and concentrated defenders.', ['siege', 'ranged'], { counters: ['frontline'], vulnerableTo: ['shock', 'pursuit'], upgradeFamily: 'siege', productionDiscountFamily: 'classical-siege' }),
  trebuchet: role('siege', 'Slow siege engine that hits cities harder but is weak against units.', ['siege', 'ranged'], { counters: ['frontline'], vulnerableTo: ['shock', 'pursuit'], upgradeFamily: 'siege', productionDiscountFamily: 'classical-siege', publicFacts: ['+25% city assault strength', '−20% unit combat strength'] }),
  ballista: role('ranged', 'Long-range bolt thrower that punishes exposed unit formations.', ['ranged', 'capture'], { counters: ['frontline'], vulnerableTo: ['shock', 'pursuit'], upgradeFamily: 'siege', productionDiscountFamily: 'classical-siege' }),
  ...
  mechanized_infantry: role('frontline', 'Mobile line infantry that captures and holds ground while keeping pace with armored forces.', ['ranged', 'mobile', 'capture'], { counters: ['frontline'], vulnerableTo: ['shock', 'siege'], upgradeFamily: 'line-infantry' }),
  ...
  battleship: role('capital-ship', 'Heavy naval fire support that bombards cities and coastal targets.', ['naval-combat', 'escort'], { counters: ['capital-ship'], vulnerableTo: ['capital-ship'], upgradeFamily: 'surface-warship', terminalReason: 'Missile Cruiser is future content.', publicFacts: ['+20% attack against cities and coastal targets', 'Submarines gain +25% attack against this unit'] }),
  tank: role('shock', 'Armored breakthrough unit that crushes exposed land defenders.', ['ranged', 'mobile', 'capture'], { counters: ['frontline', 'ranged'], vulnerableTo: ['anti-armor'], upgradeFamily: 'mounted' }),
  main_battle_tank: role('shock', 'Heavy breakthrough armor that gains 10% combat strength beside one adjacent friendly line-infantry unit.', ['frontline', 'mobile', 'capture'], { counters: ['frontline', 'ranged'], vulnerableTo: ['anti-armor'], upgradeFamily: 'mounted', terminalReason: 'Era 11 armored apex that rewards one line-infantry escort.' }),
  ...
  beast_handler: role('detection', 'Mobile detection support that reveals disguised spies while staying with the formation.', ['detection'], { secondaryRoles: ['formation-support'], counters: ['formation-support'], vulnerableTo: ['ranged'], upgradeFamily: 'detection', productionDiscountFamily: 'mounted-light-support' }),
  war_elephant: role('shock', 'A powerful charger that thrives in open ground but fears polearms and rough terrain.', ['mobile', 'capture'], { counters: ['ranged', 'siege'], vulnerableTo: ['anti-mounted'], upgradeFamily: 'mounted', productionDiscountFamily: 'mounted-heavy', terminalReason: 'Era 4 shock apex with no later roster replacement.', publicFacts: ['+20% attack on open ground', '−15% attack in forest, jungle, swamp, or hills', 'Reduces non-polearm return damage by 15%', 'Spearman and Pikeman gain +35% against this unit', 'Live Ivory reduces new city production cost by 15%'] }),
