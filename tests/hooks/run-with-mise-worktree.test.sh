...
for command in 'test:fast' 'test:slow' 'test:manifest' 'test:manifest:fast' 'test:manifest:slow' 'test:watch' build build:tauri test:web-smoke verify:pr verify:push; do
  rm -f "$mise_log"
  (
    cd "$linked"
    PATH="$fake_bin:$PATH" MISE_LOG="$mise_log" \
