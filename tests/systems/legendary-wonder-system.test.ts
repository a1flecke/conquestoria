...
describe('legendary-wonder-system', () => {
  ...
  it('seeds the full approved legendary wonder roster for a newly founded city', () => {
    ...
    expect(Object.values(result.legendaryWonderProjects ?? {}).filter(project => project.cityId === 'city-river')).toHaveLength(21);
  });
  ...
  it('keeps legendary wonder invested production in sync when sabotage hits the active build', () => {
    ...
    state.civilizations.observer = {
      ...
      cities: [],
      units: [],
      techState: { completed: ['spy-networks', 'sabotage'], currentResearch: null, researchProgress: 0, researchQueue: [], trackPriorities: {} as any },
      gold: 0,
      visibility: { tiles: {} },
      score: 0,
      diplomacy: {
